...
def start_alco(lefthand, righthand):
    lefthand.send("alco")
    righthand.send("alco")

def start_sign(lefthand, righthand):
    lefthand.send("sign")
    righthand.send("sign")

def start_assemble(lefthand, righthand):
    lefthand.send("assemble")
    righthand.send("assemble")

def start_vr(lefthand, righthand):
    lefthand.send("vr")
    righthand.send("vr")
...
class Robot:
    ...
    def set_case(self):
        try:
            case_id = yumi_cases[self.currentcase]
            self.send("301 ", case_id)
        except KeyError as e:
            log.info("The choosed case is unknown")
    ...
    def set_eulerCartesian(self, pose):
        if len(pose) != 6: return False
        msg = "102 " + self.format_offs(pose)

        return self.send(msg)

    def set_offsets(self, offsets):
        if len(offsets) != 6: return False
        msg = "101 " + self.format_offs(offsets)
        #for offs in offsets: msg += str(offs) + " "
    ...
    def format_offs(self, offs):
        offs = self.check_coordinates(offs)
        msg  = ''
        for cartesian in offs[0]:
            msg += format(cartesian * self.scale_linear,  "+08.1f") + " "
        for quaternion in offs[1]:
            msg += format(quaternion*self.scale_angle, "+08.2f") + " "
        msg += "#"
        print()
        return msg

    def set_leadthrough(self, flag):
        self.send("201 ", flag)
        log.info('Leadthrough mode is set to: %b', flag)
    ...
    def check_coordinates(coordinates):
        if ((len(coordinates) == 2) and
            ...
            return [coordinates[0:3], coordinates[3:6]]
